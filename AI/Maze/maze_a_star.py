...
class Node():
    def __init__(self, state, parent, action, dist, cost):
        self.state = state
        self.parent = parent
        self.action = action
        self.dist = dist
        self.cost = cost
        self.a_star = self.dist + self.cost
...
class A_Star():
    ...
    def remove(self): #used for removing a node from frontier
        if self.empty():
            ...
            d1=self.frontier[0].a_star
            remove_node=0
            for n in range(1,len(self.frontier)):
                if self.frontier[n].a_star<=d1:
                    remove_node=n
                    d1 = self.frontier[n].a_star
...
class Maze():
    ...
    def solve(self):
        ...
        start = Node(state = self.start, parent = None, action = None, dist = abs(self.goal[1]-self.start[1])+abs(self.goal[0]-self.start[0]), cost = 0) # A_star = Manhattan Distance + Goal Cost
        frontier = A_Star()
        frontier.add(start)
        ...
        while True:
            ...
            for action, state in self.neighbors(node.state):
                if not frontier.contains_state(state) and state not in self.explored:
                    child = Node(state = state, parent = node, action = action, dist = abs(self.goal[1]-state[1])+abs(self.goal[0]-state[0]), cost = node.cost + 1)
                    frontier.add(child)
